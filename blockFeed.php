...
// if ($stmt = $mysqli->prepare("select m.id , m.msg_title , m.msg_text , u.first_name , m.msg_time  from neighbours.messages  m ,  neighbours.users u , neighbours.message_recipients r where   m.id = r.msg_id   and m.msg_by = u.id  and r.recipient_type = 'B' and r.recipient_id=  (select block_id from neighbours.users where id = '$userId')")) 
